...
static int write_uint32(Packet *p, ogg_uint32_t val) {
    ...
    p->data[p->pos] = (unsigned char) ((val) & 0xFF);
    p->data[p->pos + 1] = (unsigned char) ((val >> 8) & 0xFF);
    p->data[p->pos + 2] = (unsigned char) ((val >> 16) & 0xFF);
    p->data[p->pos + 3] = (unsigned char) ((val >> 24) & 0xFF);
    p->pos += 4;
    return 1;
}
...
static void comment_init(char **comments, int *length, const char *vendor_string) {
    ...
    memcpy(p + 12, vendor_string, (size_t) vendor_length);
...
static void comment_pad(char **comments, int *length, int amount) {
    if (amount > 0) {
        char *p = *comments;
        // Make sure there is at least amount worth of padding free,
        // and round up to the maximum that fits in the current ogg segments
        int newlen = (*length + amount + 255) / 255 * 255 - 1;
        p = realloc(p, (size_t) newlen);
        int i = 0;
...
static int writeOggPage(ogg_page *page, FILE *os) {
    int written = (int) fwrite(page->header, sizeof(unsigned char), (size_t) page->header_len, os);
...
int initRecorder(const char *path) {
    ...
    _packet = malloc((size_t) max_frame_bytes);
...
int writeFrame(uint8_t *framePcmBytes, unsigned int frameByteCount) {
    ...
    if (nb_samples != 0) {
        ...
        if (nb_samples < cur_frame_size) {
            paddedFrameBytes = malloc((size_t) (cur_frame_size * 2));
            freePaddedFrameBytes = 1;
            memcpy(paddedFrameBytes, framePcmBytes, frameByteCount);
            memset(paddedFrameBytes + nb_samples * 2, 0, (size_t) (cur_frame_size * 2 - nb_samples * 2));
        }
    ...
    while ((((size_segments <= 255) && (last_segments + size_segments > 255)) ||
            (enc_granulepos - last_granulepos > max_ogg_delay)) && ogg_stream_flush_fill(&os, &og, 255 * 255)) {
